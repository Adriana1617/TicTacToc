...
contract TicTacToc{
    ...
    function jugar(uint idPartida, uint horizontal , uint vertical) public {
        ...
        //Guardar la jugada 
        guardaMovimiento(idPartida, horizontal, vertical);
        partida = partidas[idPartida]; 


        //Verificar si hya un ganador o esta llena 
        uint ganador = obtenerGanador(partida); 
        guardarGanador(ganador, idPartida, partida);

        partidas[idPartida].ultimoTurno = msg.sender; 
    }

    function guardarGanador(uint ganador, uint idPartida ,Partida memory partida) private {
        if(ganador != 0){
            ...
            for(uint x =1; x < 4; x ++)
            {
                for (uint y = 1; y < 4 ; y ++)
                {
                     if (partida.jugadas[x][y] == 0) casillasDisponibles = true;  
                }
            }
    ...
    function  guardaMovimiento(uint idPartida, uint h, uint v) private{
